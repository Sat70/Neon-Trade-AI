...
function deriveSignalFromPredictionPayload(ticker: string, horizon: string, payload: unknown) {
  const parsed = payload as { predictions?: PredictionPoint[] }
  ...
  if (expectedReturn > 1.5) signal = 'Bullish'
  else if (expectedReturn < -1.5) signal = 'Bearish'

  const confidence = Math.max(55, Math.min(95, Math.round(70 + Math.abs(expectedReturn) * 2)))
  const riskLevel: 'Low' | 'Medium' | 'High' =
    Math.abs(expectedReturn) < 4 ? 'Low' : Math.abs(expectedReturn) < 10 ? 'Medium' : 'High'
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        const predictResponse = await fetch(
          `${ML_SERVICE_BASE_URL}/predict?ticker=${encodeURIComponent(tickerParam)}&horizon=${encodeURIComponent(horizonParam)}`,
