...
router.get('/', async (req, res) => {
    ...
    const horizonParam = String(req.query.horizon ?? 'intraday');
    ...
    try {
        ...
        const response = await fetch(`${ML_SERVICE_BASE_URL}/timing?ticker=${encodeURIComponent(tickerParam)}&horizon=${encodeURIComponent(horizonParam)}`, { signal: controller.signal });
        ...
        return res.status(502).json({
            message: 'Failed to reach ML timing service.',
            details: error instanceof Error ? error.message : 'Unknown error',
        });
    }
});
exports.default = router;
//# sourceMappingURL=timingRoutes.js.map